import type { Location } from '../api/rickAndMorty';
...
export default function LocationCard({ location, index }: LocationCardProps) {
  const idLabel = String(location.id).padStart(3, '0');
  const meta: Array<[string, string]> = [
    ['Tipo', location.type || 'Desconocido'],
    ['Dimensión', location.dimension || 'Desconocida'],
    ['Residentes', String(location.residents.length)],
  ];
